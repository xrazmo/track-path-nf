...
def insert_data(conn, table_name, df, add_timestamp=True):
    """Efficiently insert data from dataframe into SQLite table with timestamp and duplicate checking."""
    if df.empty:
        print("No data to insert.")
        return 0

    # Add import timestamp if needed (do this once, not per row)
    if add_timestamp:
        current_time = datetime.now().isoformat()
        df['import_timestamp'] = current_time

    # Calculate record hashes only if not already present
    if 'record_hash' not in df.columns:
        # Vectorized hash calculation instead of row-by-row apply
        df_for_hash = df.copy()
        if 'exists' in df_for_hash.columns:
            df_for_hash = df_for_hash.drop('exists', axis=1)
        if 'import_timestamp' in df_for_hash.columns:
            df_for_hash = df_for_hash.drop('import_timestamp', axis=1)
            
        # Convert all columns to string and concatenate - much faster than apply
        hash_input = df_for_hash.astype(str).values.sum(axis=1)
        df['record_hash'] = [hashlib.md5(val.encode()).hexdigest() for val in hash_input]

    # Fetch existing hashes using a temporary table for faster comparison
    cursor = conn.cursor()
    
    # Create a temporary table with just the hashes from the dataframe
    temp_table = f"temp_hashes_{table_name}"
    cursor.execute(f"CREATE TEMPORARY TABLE IF NOT EXISTS {temp_table} (record_hash TEXT PRIMARY KEY)")
    
    # Bulk insert hashes to temporary table
    hash_values = [(h,) for h in df['record_hash'].tolist()]
    cursor.executemany(f"INSERT OR IGNORE INTO {temp_table} VALUES (?)", hash_values)
    
    # Get new hashes using SQL (much faster than Python filtering)
    query = f"""
    SELECT t.record_hash 
    FROM {temp_table} t
    LEFT JOIN "{table_name}" m ON t.record_hash = m.record_hash
    WHERE m.record_hash IS NULL
    """
    new_hashes = {row[0] for row in cursor.execute(query)}
    
    # Filter dataframe (if there are any new records)
    if new_hashes:
        new_df = df[df['record_hash'].isin(new_hashes)]
    else:
        new_df = df.iloc[0:0]  # Empty dataframe with same structure
    
    # Clean up temporary table
    cursor.execute(f"DROP TABLE IF EXISTS {temp_table}")
    
    if new_df.empty:
        print(f"Processed {len(df)} records, inserted 0 new records.")
        return 0

    # Remove columns not in table
    if 'exists' in new_df.columns:
        new_df.drop('exists', axis=1, inplace=True)

    # Use executemany with parameters for batch insert
    cols = list(new_df.columns)
    placeholders = ','.join(['?'] * len(cols))
    col_str = ','.join([f'"{c}"' for c in cols])
    insert_sql = f'INSERT INTO "{table_name}" ({col_str}) VALUES ({placeholders})'

    # Convert to list of tuples for faster insert
    values = list(new_df.itertuples(index=False, name=None))
    conn.executemany(insert_sql, values)
    conn.commit()

    print(f"Processed {len(df)} records, inserted {len(new_df)} new records.")
    return len(new_df)
...
def fetch_amrfinder_data(input_dir):
    ...
    return pd.concat(amrfinder_data, ignore_index=True) if amrfinder_data else pd.DataFrame()
...
def save_to_db(input_dir, output_dir, db_name, add_seq = False,cpus=1):
...
def main():
    ...
    save_to_db(input_dir, output_dir, db_name,args.add_seq,args.cpus)
