...
usage() {
    ...
    echo "  -f, --free_param    Arbitrary parameters to append directly to the Nextflow command"
...
ARGS=$(getopt -o q:g:s:o:w:c:p:rf:h --long reads_dir:,contigs_dir:,species_csv:,output:,work:,config:,profile:,resume,free_param:,help -n "$0" -- "$@")
...
echo -e "Free Parameters:       ${GREEN}${FREE_PARAM:-None}${NC}"
...
[ -n "$FREE_PARAM" ] && NEXTFLOW_CMD="$NEXTFLOW_CMD $FREE_PARAM"
